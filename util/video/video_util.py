        ...
        with console.status('[cyan]Processing... please wait!', spinner='line') as status:
            ...
            for file in clip_list:    
                ...
                if process_function == 1:
                    video_editor.flip(input_path=input_path, output_path=output, threads=threads, preset=preset)  
                elif process_function == 2:
                    video_editor.change_speed(input_path=input_path, speed_factor=speed_factor, output_path=output, threads=threads, preset=preset) 
                elif process_function == 3:
                    video_editor.flip_and_change_speed(input_path=input_path, speed_factor=speed_factor, output_path=output, threads=threads, preset=preset) 
                elif process_function == 4:
                    video_editor.add_background_music(input_path=input_path, music_path=music_path, output_path=output, threads=threads, preset=preset) 
                elif process_function == 5:
                    video_editor.change_speed_and_add_music(input_path=input_path, output_path=output, music_path=music_path, speed_factor=speed_factor, threads=threads, preset=preset) 
                elif process_function == 6:
                    video_editor.change_speed_add_music_and_flip(input_path=input_path, output_path=output, music_path=music_path, speed_factor=speed_factor, tthreads=threads, preset=preset) 
                elif process_function == 7:
                    video_editor.color_correction(input_path=input_path, output_path=output, threads=threads, preset=preset) 
                end = time.time()
