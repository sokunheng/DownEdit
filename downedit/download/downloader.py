...
class Download:
    ...
    @staticmethod
    def download_image(folder_path, download_url, file_name):
        ...
        cleaned_name = re.sub(r'["*<>?\\|/:]', '', file_name)
        file_path = os.path.join(folder_path, f"{cleaned_name}.jpg")
        
        chunk_size = 1024
        size = 0
        if not os.path.exists(file_path):
            image_bytes = requests.get(download_url, stream=True)
            total_length = int(image_bytes.headers.get("Content-Length"))
            
            with open(file_path, 'wb') as out_file:
                
                for data in image_bytes.iter_content(chunk_size=chunk_size):
                    size += len(data)
                    print('\r' + f"{Fore.CYAN}[Programs] {Fore.GREEN}[Download]{Fore.WHITE} " + '%s %.2f%%' % (
                        '>' * int(size * 50 / total_length), float(size / total_length * 100)), end=' ')
                    out_file.write(data)
                ...
                print("")
                print(
                    f"""{Fore.CYAN}[Programs] {Fore.YELLOW}[File] {Fore.GREEN}{file_name}.jpg{Fore.YELLOW} Downloaded\n""")
                time.sleep(0.5)

        else:
            print(
                f"{Fore.CYAN}[Programs] {Fore.YELLOW}[File] {Fore.GREEN}{file_name}.jpg{Fore.WHITE} already exists! Skipping...\n")
            time.sleep(0.5)
